...
routinely asserted without measurement. The metrics RL borrows from NLP are themselves under
indictment --- existing faithfulness tests have been shown to measure self-consistency rather
than faithfulness \citep{parcalabescu2024} --- and saliency-style post-hoc evidence is
exploratory, not explanatory \citep{atrey2020}. Meanwhile the belief-state line has descriptive
maturity and a prescriptive vacuum: recurrent policies provably approximate belief states
\citep{lambrechts2022}, yet no deployed policy we know of makes its belief a named, auditable,
\emph{writable} interface whose stated belief is certified to be the one it acts on.
\textbf{This paper answers all three: \textsc{CrystalScore} is a loseable evaluation protocol
applied identically to a black box, its best post-hoc program, and a born-legible successor on
one real high-stakes substrate --- the instrument the surveys ask for; its application yields
the discriminating-axis finding (faithfulness does not separate the architectures;
parity-constrained simulatability does); and CRYSTAL-1's regime posterior is, to our knowledge,
the first certified named-and-writable belief interface of a deployed policy
(\S\ref{sec:crystal}).}
...
The ablation that justifies the name is the honest part
(Figure~\ref{fig:chrlablation}, Table~\ref{tab:chrlablation}). The flat baseline wins raw
return ($15.9\%$) and even Sharpe ($1.14$) --- CHRL never claims to out-earn it. Hierarchy
\emph{alone} is the worst arm on the metric that matters (return/drawdown $0.79$, with $-13.0\%$
drawdown and $1.8\%$/day turnover): structure without discipline just adds churn. Constraints
\emph{alone} do the heavy lifting on risk ($-6.3\%$ drawdown, $0.6\%$/day turnover) at a return
cost. Only the full combination recovers performance while keeping risk controlled
(return/drawdown $1.37$, the best of the four). The load-bearing ingredient is the constraint
set --- the hierarchy is the scaffolding that makes the constraints expressible and the
decisions loggable, and it pays off only \emph{inside} them.
...
\bottomrule
\end{tabular}
\caption{The four-way ablation on 4 walk-forward validation folds. Best per column in bold: the
flat baseline keeps raw return and Sharpe; constraints own the risk columns; the full
combination wins the ratio. Hierarchy alone is the \emph{worst} arm --- the honest reading is
that constraints, not hierarchy, buy the risk-adjusted result.}
...
\begin{figure}[t]
  ...
  \caption{The same ablation, panel per metric. Constraints cut drawdown and turnover roughly
  in half; hierarchy alone hurts; full CHRL recovers performance while keeping risk
  controlled.}
